...
export class CalendarComponent implements OnInit {
  ...
  // 目前先寫死 groupId，之後登入後可改成從使用者家庭群組取得
  groupId = 1;

  // 目前先寫死 createdBy，之後登入後可改成目前登入者 userId
  createdBy = 1;
 ...
 calendarOptions: CalendarOptions = {
  ...
  loadCalendarEvents(): void {
    this.calendarApiService.getByGroup(this.groupId).subscribe({
      next: (res) => {
...
preConfirm: () => {
  ...
  const description = (document.getElementById('createDescription') as HTMLInputElement).value;
  const eventDate = (document.getElementById('createEventDate') as HTMLInputElement).value;
  const eventTime = (document.getElementById('createEventTime') as HTMLInputElement).value;
  const endDate = (document.getElementById('createEndDate') as HTMLInputElement).value;
  const endTime = (document.getElementById('createEndTime') as HTMLInputElement).value;
  const notifyBefore = Number((document.getElementById('createNotifyBefore') as HTMLInputElement).value);
...
preConfirm: () => {
  ...
  const eventDate = (document.getElementById('updateEventDate') as HTMLInputElement).value;
  const eventTime = (document.getElementById('updateEventTime') as HTMLInputElement).value;
  const endDate = (document.getElementById('updateEndDate') as HTMLInputElement).value;
  const endTime = (document.getElementById('updateEndTime') as HTMLInputElement).value;
  const notifyBefore = Number((document.getElementById('updateNotifyBefore') as HTMLInputElement).value);
  const startDateTime = `${eventDate}T${eventTime}:00`;
  const endDateTime = `${endDate}T${endTime}:00`;
  const today = new Date();
  today.setHours(0, 0, 0, 0);
...
handleEventDrop(info: any): void {
  ...
  const newDateTime = info.event.startStr.length === 10
